// =============================================================================
// DECEL Market Opportunity Intelligence — UNVERIFIED TREATMENT (v1.1.4)
//
// Hitank 2026-09-14 / Cassin 2026-09-11: "Delete the fabricated market table.
// The frontend bundle holds 50 countries as literals, and ge() computes
// value as activeYards * 70 to activeYards * 150 MSEK. The counts have no
// source. The UK is listed at 8 yards and High priority; the UK has no
// marshalling yards in operation. Do: delete it. If it must survive for
// the demo, every figure renders with an unmissable 'unverified estimate'
// treatment. Why now: a fabricated number said out loud at a trade fair
// cannot be withdrawn."
//
// What changed (Phase 0 #1):
//   1. Every MarketOpportunity now carries an `unverified: true` flag
//      (NO country has a sourced fact for activeYards yet).
//   2. potentialValueMinMSEK / Max are computed but ONLY displayed when
//      unverified=false. The ge() formula is unchanged; we just hide
//      it behind a verification gate so the operator can't accidentally
//      quote a fabricated number.
//   3. The UI in ResultCard + GlobalRadar + Dossier must render the
//      "⚠ UNVERIFIED ESTIMATE" treatment whenever unverified=true. The
//      treatment: red-bordered box, "Unverified — do not quote" line,
//      and the value line is greyed out with strikethrough.
//   4. The 50 country literals stay in `MARKET_DATA` because the country
//      picker in CountryScanner / GlobalRadar depends on them. But the
//      only thing rendered from each entry is its name + ISO country
//      code; the yards / value / readiness / priority are all gated.
//
// Done when: no number appears in the UI that cannot be traced to a
// source (Cassin). For v1: nothing is traceable, so EVERY market number
// gets the unverified treatment. The only exception is a market that
// gets a SourcedFact-backed yard count from the live DB (see
// `listOrgs + market_ids` + the dossiers route) — those override the
// MARKET_DATA entry entirely.
// =============================================================================
...
export interface MarketOpportunity {
  /** Active hump yards, per Cassin's old (unverified) intelligence. */
  activeYards: number;
  /** Potential order value min, MSEK. Derived from activeYards × 70 (internal benchmark). */
  potentialValueMinMSEK: number;
  /** Potential order value max, MSEK. Derived from activeYards × 150 (internal benchmark). */
  potentialValueMaxMSEK: number;
  readiness: MarketReadiness;
  priority: MarketPriority;
  rationale: string;
  /**
   * Always true for the legacy MARKET_DATA entries — they have no source.
   * Set to false ONLY when a real SourcedFact-backed yard count is loaded
   * from the live DB (see dossier.tsx / ResultCard override logic).
   */
  unverified: boolean;
}
...
/**
 * 50-country literal list. Kept so the country picker still works, but
 * every entry is `unverified: true` (no sourced yard count). The picker
 * must NOT show any number from this — only the country name.
 *
 * The UK entry was the canary: it claimed 8 yards at High priority,
 * which is wrong (UK has 0 marshalling yards in operation). Per Cassin's
 * rule, all of this is now hidden behind the "UNVERIFIED ESTIMATE"
 * treatment until a real source is wired in (Phase 2 #22 + #23).
 */
...
/** Lookup the hardcoded (UNVERIFIED) market entry. UI must check unverified=true. */
 ...
 * count (loaded from the live DB). This is the path that overrides the
 * legacy MARKET_DATA entry when the dossier has a sourced count.
 *
 * Used by: dossier.tsx, ResultCard.tsx (override path).
 */
export function buildVerifiedMarket(
  ...
  return {
    ...
    priority: "Medium",
    rationale,
    unverified: false, // SourcedFact-backed — UI may show the value
  };
}
